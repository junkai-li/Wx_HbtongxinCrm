...
namespace Cms.Controllers
{
    public class MemberController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetMemberList(int start, int length)
        {
            using (var db = new dbContext())
            {
                var search = Request.Query["search[value]"].ToString();
                var query = db.TMember.Include(x => x.CreateUser).Where(t => t.IsDelete == false);
                var recordsTotal = query.Count();
                if (!string.IsNullOrEmpty(search))
                {
                    query = query.Where(t => t.ChildName.Contains(search) || t.ParentName.Contains(search) || t.PhoneNumber.Contains(search) || t.CourseName.Contains(search) || t.SchoolName.Contains(search));
                }
        ...
        public bool MemberSave(TMember user)
        {
            ...
            using (var db = new dbContext())
            {
                ...
                else
                {
                    ...
                    dbUserSys.CourseName = user.CourseName;
